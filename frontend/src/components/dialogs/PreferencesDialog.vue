...
const langOption: LangOption[] = Object.entries(lang).map(([key, value]) => ({
  value: key,
...
const applyPreferences = (pf: Preferences) => {
...
const onRestoreDefaults = async () => {
  const pf: Preferences = await RestoreDefault()
  applyPreferences(pf)
}
