...
import { NIcon, useDialog, useMessage, type TreeOption, type DropdownOption } from 'naive-ui'
...
// 类型定义
// interface MenuOption extends DropdownOption {
//   key: string
//   label: string
//   icon?: () => VNodeChild
// }
...
const contextMenuOptions = ref<MenuOption[] | null>(null)
...
// 菜单选项配置
const menuOptions: Record<ConnectionType, (option: TreeNode) => MenuOption[]> = {
...
// 事件处理函数
const onUpdateExpanded = (value: string[], option: TreeNode, meta: TreeMeta): void => {
...
const renderContextLabel = (option: MenuOption): VNodeChild => {
  return h('div', { class: 'context-menu-item' }, option.label)
}

const renderPrefix = ({ option }: { option: TreeNode }): VNodeChild => {
  switch (option.type) {
...
const renderLabel = ({ option }: { option: TreeNode }): string => {
  switch (option.type) {
    case ConnectionType.RedisDB:
    case ConnectionType.RedisKey:
...
const renderSuffix = ({ option }: { option: TreeNode }): VNodeChild => {
  // 可以在这里添加后缀渲染逻辑
  return null
}

const nodeProps = ({ option }: { option: TreeNode }) => {
  return {
    onClick(): void {
      connectionStore.select({key: option.key,
        name: option.name as string,
        db: option.db as number, type: option.type, redisKey: option.redisKey as  string})
    },
...
const onLoadTree = async (node: TreeNode): Promise<void> => {
  switch (node.type) {
    case ConnectionType.Server:
      loading.value = true
      try {
