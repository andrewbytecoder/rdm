<script setup lang="ts">
import { computed } from 'vue'

// 定义 emits 类型
const emit = defineEmits<{
  (e: 'update:modelValue', value: boolean): void
}>()

// 定义 props 类型
interface Props {
  modelValue?: boolean
  clickToggle?: boolean
  strokeWidth?: number | string
  fillColor?: string
  size?: number | string
}

const props = withDefaults(defineProps<Props>(), {
  modelValue: false,
  clickToggle: false,
  strokeWidth: 3,
  fillColor: '#dc423c',
  size: 24
})

// 处理切换逻辑
const onToggle = () => {
  if (props.clickToggle) {
    emit('update:modelValue', !props.modelValue)
  }
}

// 计算当前是否激活状态
const isActive = computed(() => props.modelValue)
</script>

<template>
  <svg
      v-if="isActive"
      :height="size"
      :width="size"
      fill="none"
      viewBox="0 0 48 48"
      xmlns="http://www.w3.org/2000/svg"
      @click="onToggle"
      style="cursor: pointer;"
  >
    <path
        :stroke="fillColor"
        :stroke-width="strokeWidth"
        d="M41 4H7C5.34315 4 4 5.34315 4 7V41C4 42.6569 5.34315 44 7 44H41C42.6569 44 44 42.6569 44 41V7C44 5.34315 42.6569 4 41 4Z"
        fill="#dc423c"
        stroke-linecap="round"
        stroke-linejoin="round"
    />
    <path :stroke-width="strokeWidth" d="M4 32H44" stroke="#FFF" stroke-linecap="round" />
    <path
        :stroke-width="strokeWidth"
        d="M10 38H11"
        stroke="#FFF"
        stroke-linecap="round"
        stroke-linejoin="round"
    />
    <path
        :stroke-width="strokeWidth"
        d="M26 38H38"
        stroke="#FFF"
        stroke-linecap="round"
        stroke-linejoin="round"
    />
    <path
        :stroke="fillColor"
        :stroke-width="strokeWidth"
        d="M44 37V27"
        stroke-linecap="round"
        stroke-linejoin="round"
    />
    <path
        :stroke="fillColor"
        :stroke-width="strokeWidth"
        d="M4 37V27"
        stroke-linecap="round"
        stroke-linejoin="round"
    />
  </svg>
  <svg
      v-else
      :height="size"
      :width="size"
      fill="none"
      viewBox="0 0 48 48"
      xmlns="http://www.w3.org/2000/svg"
      @click="onToggle"
      style="cursor: pointer;"
  >
    <path
        :stroke-width="strokeWidth"
        d="M41 4H7C5.34315 4 4 5.34315 4 7V41C4 42.6569 5.34315 44 7 44H41C42.6569 44 44 42.6569 44 41V7C44 5.34315 42.6569 4 41 4Z"
        fill="none"
        stroke="currentColor"
        stroke-linecap="round"
        stroke-linejoin="round"
    />
    <path :stroke-width="strokeWidth" d="M4 32H44" stroke="currentColor" stroke-linecap="round" />
    <path
        :stroke-width="strokeWidth"
        d="M10 38H11"
        stroke="currentColor"
        stroke-linecap="round"
        stroke-linejoin="round"
    />
    <path
        :stroke-width="strokeWidth"
        d="M26 38H38"
        stroke="currentColor"
        stroke-linecap="round"
        stroke-linejoin="round"
    />
    <path
        :stroke-width="strokeWidth"
        d="M44 37V27"
        stroke="currentColor"
        stroke-linecap="round"
        stroke-linejoin="round"
    />
    <path
        :stroke-width="strokeWidth"
        d="M4 37V27"
        stroke="currentColor"
        stroke-linecap="round"
        stroke-linejoin="round"
    />
  </svg>
</template>

<style lang="scss" scoped></style>
